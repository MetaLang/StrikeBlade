...
globalvar __ls_draw_mode,__ls_isenabled,__ls_ambient;
...
/*
    ...
    static
*/
return (__ls_light_create(argument0,argument1,
                          argument2,argument6,
                          argument4,argument5,
                          argument7,argument10,
                          argument8,argument3,
                          argument9));
...
ls_light_surface_draw(argument0,1);
#define ls_light_surface_draw_NEW
/*
    Arguments:
    0: id
    1: update if static?
...
/*
    Grab the light's properties first.
*/
var _ssurf=ds_list_find_value(argument0,ds_list_size(argument0)-1),/*Shadow surface*/
    _lsurf=ds_list_find_value(argument0,ds_list_size(argument0)-4),/*Light surface*/
    _rad=ds_list_find_value(argument0,2),/*Light radius*/
    _dir=ds_list_find_value(argument0,4),/*Direction of light*/
    _spread=ds_list_find_value(argument0,3),/*Spread of the light*/
    _prec=ds_list_find_value(argument0,6),/*Light Precision*/
    _corerad=ds_list_find_value(argument0,9),/*Radius of core*/
    _ldepth=ds_list_find_value(argument0,10);/*The height of the light*/
    
var updated;
updated=argument1;

/*If the shadow surface was destroyed, recreate it.*/
if (!surface_exists(_ssurf))
{
    _ssurf=surface_create(rad*2,rad*2);
    ds_list_replace(argument0,ds_list_size(argument0)-1,_ssurf);
    updated=true;
}

/*If static and we don't need an update, we just quit.*/
if (ds_list_find_value(argument0,7)&&!updated)
    return 0;
    
/*If the light surface was destroyed, marked as updated, we recreate it.*/
var lupdated=false;
if (!surface_exists(_lsurf) || _lsurf==-1)
{
    _lsurf=surface_create(rad*2,rad*2);
    ds_list_replace(argument0,ds_list_size(argument0)-4,_lsurf);
    lupdated=true;
}

/*If we need to update the light surface, we redraw the light.*/
if (ds_list_find_value(argument0,ds_list_size(argument0)-3)||lupdated)
{
    /*We mark the light as updated.*/
    ds_list_replace(argument0,ds_list_size(argument0)-3,false);
    ...
    surface_set_target(_lsurf);
    draw_clear(c_black);
    /*Draw the fade of the light*/
    draw_primitive_begin_texture(pr_trianglestrip,
                                 sprite_get_texture(__ls_penumbra_s,0));
                                 
    draw_vertex_texture_color(_rad+cos(pi/180*(_dir-_spread/2))*_corerad,_rad-sin(pi/180*(_dir-_spread/2))*_corerad,soft_edge,1,c_white,1);
    draw_vertex_texture_color(_rad+cos(pi/180*(_dir-_spread/2))*_rad,_rad-sin(pi/180*(_dir-_spread/2))*_rad,soft_edge,0,c_black,1);
    for (var i=_dir-_spread/2+_spread/_prec;i<=_dir+_spread/2-_spread/_prec;
         i+=_spread/_prec)
    {
        CRx=cos(pi/180*i)*_corerad;
        CRy=-sin(pi/180*i)*_corerad;
        draw_vertex_texture_color(_rad+CRx,_rad+CRy,0,1,c_white,1);
        draw_vertex_texture_color(_rad+cos(pi/180*i)*_rad,_rad-sin(pi/180*i)*_rad,0,0,c_black,1);
    }
    draw_vertex_texture_color(_rad+cos(pi/180*(_dir+_spread/2-_spread/_prec))*_corerad,_rad-sin(pi/180*(_dir+_spread/2-_spread/_prec))*_corerad,0,1,c_white,1);
    draw_vertex_texture_color(_rad+cos(pi/180*(_dir+_spread/2-_spread/_prec))*_rad,_rad-sin(pi/180*(_dir+_spread/2-_spread/_prec))*_rad,0,0,c_black,1);

    if (soft_edge)
        draw_vertex_texture_color(_rad+cos(pi/180*(_dir+_spread/2))*_rad,_rad-sin(pi/180*(_dir+_spread/2))*_rad,soft_edge,0,c_black,1);
    else
    {
        draw_vertex_texture_color(_rad+cos(pi/180*(_dir-_spread/2))*_corerad,_rad-sin(pi/180*(_dir-_spread/2))*_corerad,0,1,c_white,1);
        draw_vertex_texture_color(_rad+cos(pi/180*(_dir-_spread/2))*_rad,_rad-sin(pi/180*(_dir-_spread/2))*_rad,0,0,c_black,1);
    }
    ...
    if (_corerad>0)
    {
        ...
        for (var i=_dir-_spread/2;i<=_dir+_spread/2;i+=_spread/_prec)
        {
            if ((i!=_dir-_spread/2&&i!=_dir+_spread/2)||!soft_edge)
                draw_vertex(_rad+cos(pi/180*i)*_corerad,_rad-sin(pi/180*i)*_corerad);
        }
        draw_primitive_end();
    }
...
/*Prepare to draw the stencil shadows.*/
surface_set_target(_ssurf);
draw_clear(c_white);

var _size=ds_list_size(__ls_list_shadow_positions),/*Number of shadows to consider*/
    light_x=ds_list_find_value(argument0,0),/*Light x/y*/
    light_y=ds_list_find_value(argument0,1),
    _shads=ds_list_find_value(argument0,8),/*Whether shadows are enabled/disabled*/
    _shadow_list=ds_list_create();/*Which shadow ID's to draw*/
    
if (_shads)
{
    /*Determine which shadows need drawing*/
    for (var i=0;i<_size;++i)
    {
        //Grabs the ID. The sign determines culling.
        var _sID=ds_list_find_value(__ls_list_shadow_positions,i);
        var _ID=abs(ds_list_find_value(abs(_sID),0)),
            X=ds_list_find_value(abs(_sID),1),
            Y=ds_list_find_value(abs(_sID),2);
        var _points=ds_grid_height(_ID)-1;
        /*We need to disable shadows outside the light.
          We keep a shape if at least one point is inside.*/
        var addshadow=false;
        for (var j=0;j<_points;++j)
        {
            var pt=ds_grid_get(_ID,0,j),
                pr=ds_grid_get(_ID,1,j);
            if (point_distance(light_x,light_y,
                               X+cos(pi/180*pt)*pr,
                               Y-sin(pi/180*pt)*pr)<=_rad*1.5)
            {
                addshadow=true;
                break;
            }
        ...
        if (addshadow)
            ds_list_add(_shadow_list,_sID);
    }
    /*Draw the stencil shadows*/
    for (var i=0;i<ds_list_size(_shadow_list);++i)
    {
        ...
        var ID=abs(ds_list_find_value(abs(cullingon),0)),
            ...
            depthlayer=ds_list_find_value(abs(cullingon),3),
            capped=ds_list_find_value(abs(cullingon),4);
        ...
        {
            /*If no culling, we add all the points.*/
        ...
        repeat (1+__ls_debug)
        {
            ...
            for (var j=0;j<points;++j)
            {
                ...
                /*Here we handle overhead vs same-level lights */   
                var drawdis=_rad*5;
                var drawdis_p=_rad*5;
                if (_ldepth>depthlayer)
                {
                    drawdis=point_distance(X+cos(pi/180*pt)*pr,Y-sin(pi/180*pt)*pr,
                                           light_x,light_y);
                    drawdis_p=point_distance(X+cos(pi/180*pt_p)*pr_p,Y-sin(pi/180*pt_p)*pr_p,
                                           light_x,light_y);
                }
                ...
                if (ds_list_find_index(points_todraw,j)!=-1)
                {
                    ...
                    draw_vertex_color(XX+cos(pi/180*pt_p)*pr_p+cos(pi/180*dir_p)*drawdis_p,
                                      YY-sin(pi/180*pt_p)*pr_p-sin(pi/180*dir_p)*drawdis_p,__color,1);
                    
                    draw_vertex_color(XX+cos(pi/180*pt)*pr,YY-sin(pi/180*pt)*pr,__color,1);
                    if (rep__==0)
                        draw_vertex_color(XX+cos(pi/180*pt_p)*pr_p+cos(pi/180*dir_p)*drawdis_p,
                                          YY-sin(pi/180*pt_p)*pr_p-sin(pi/180*dir_p)*drawdis_p,__color,1);
                    draw_vertex_color(XX+cos(pi/180*pt)*pr+cos(pi/180*dir)*drawdis,
                                      YY-sin(pi/180*pt)*pr-sin(pi/180*dir)*drawdis,__color,1);
                }
            }
            draw_primitive_end();
            ++rep__;
...
var surf=ds_list_find_value(argument[0],ds_list_size(argument[0])-1),
    ...
    lightdepthlayer=ds_list_find_value(argument[0],10);
...
//This list will store which shadows are drawn. This way we don't have to
//recalculate it for the "above caps".
var listforcaps=ds_list_create();
if (_shads)
for (var i=0;i<size;++i)
{
    ...
    var cullingon=ds_list_find_value(__ls_list_shadow_positions,i);
    var culORIG=cullingon;//Needed for "light above" caps below. Messy, but it works.
    var ID=abs(ds_list_find_value(abs(cullingon),0)),
        X=ds_list_find_value(abs(cullingon),1),
        Y=ds_list_find_value(abs(cullingon),2),
        depthlayer=ds_list_find_value(abs(cullingon),3),
    ...
    */
    ...
    //For use on the last for loop outside this scope.
    if (drawthings)
        ds_list_add(listforcaps,culORIG);
    ...
    repeat (1+__ls_debug)
    {
        ...
        for (j=0;j<points;++j)
        {
            ...
            var dir=point_direction(light_x,light_y,X+cos(pi/180*pt)*pr,Y-sin(pi/180*pt)*pr),
                ...
                if (capped&&j>1&&rep__==0&&lightdepthlayer<=depthlayer)
                {
...
//This is for the light caps when drawing lights ABOVE shadow objects.

for (var i=0;i<ds_list_size(listforcaps);++i)
{
        draw_primitive_begin(pr_trianglelist);
        var cullingon=ds_list_find_value(listforcaps,i);
        var ID=abs(ds_list_find_value(abs(cullingon),0)),
            X=ds_list_find_value(abs(cullingon),1),
            Y=ds_list_find_value(abs(cullingon),2),
            depthlayer=ds_list_find_value(abs(cullingon),3),
        ...
        for (j=0;j<points;++j)
        {
            ...
            var XX,YY;
                XX=X-(light_x-rad);
                YY=Y-(light_y-rad);
            if (j>1&&capped&&lightdepthlayer>depthlayer)
            {
                draw_vertex_color(XX+cos(pi/180*pt_f)*pr_f,YY-sin(pi/180*pt_f)*pr_f,c_white,1);
                draw_vertex_color(XX+cos(pi/180*pt_p)*pr_p,YY-sin(pi/180*pt_p)*pr_p,c_white,1);
                draw_vertex_color(XX+cos(pi/180*pt)*pr,YY-sin(pi/180*pt)*pr,c_white,1);
            }
        }
        draw_primitive_end();
}
ds_list_destroy(listforcaps);
draw_set_color(c_white);

/*Moved the light drawing down here so that we can draw caps without messing the light.*/
draw_set_blend_mode_ext(bm_inv_dest_alpha,bm_src_color);
draw_surface(tsurf,0,0);
/*Draw the light onto it.*/
draw_set_blend_mode(bm_normal);
surface_reset_target();
...
ds_list_add(ID,argument3);
ds_list_add(ID,argument4);
...
var a, b, d, dp, xx, yy, returnX, returnY;
xx = argument0 - argument3;
yy = argument1 - argument4;
dp = xx * (cos(pi/180*argument5))+ yy * -sin(pi/180*argument5);
d = dp + sqrt(sqr(dp) + sqr(argument2) - sqr(xx) - sqr(yy))
returnX = argument3 + (cos(pi/180*argument5)) * d
returnY = argument4 - (sin(pi/180*argument5)) * d
